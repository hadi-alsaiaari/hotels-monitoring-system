							...
							<div class="card-body">
								<div class="table-responsive">
									<table id="example" class="table key-buttons text-md-nowrap">
										...
										<tbody>
											@foreach ($firearms as $n => $firearm)
											<tr>
												<td class="sorting_1">{{$n+1}}</td>
												<td>{{$firearm->firearm_serial_number}}</td>
												<td>{{$firearm->firearm_type}}</td>
												<td >{{$firearm->license_number}}</td>
												<td>{{$firearm->license_type}}</td>
												<td>
													@can('firearms.view')
														<a href="{{route("{$x}_firearm.show",$firearm->id )}}"  class="btn btn-warning me-1 link-icon"><i data-feather="eye" ></i></a>	
													@endcan
												</td>
